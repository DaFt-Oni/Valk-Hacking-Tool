...
def nfc_pc_tools():
    while True:
        clear_screen()
        display_banner()
        print(f"{Colors.BOLD}{Colors.OKBLUE}Modo PC con dispositivo NFC{Colors.ENDC}")
        ...
        if opcion == '1':
            with nfc.ContactlessFrontend('usb') as clf:
                print(f"\n{Colors.OKGREEN}[+] Acerca un tag NFC al lector...{Colors.ENDC}")
            ...
            with nfc.ContactlessFrontend('usb') as clf:
                print(f"\n{Colors.OKGREEN}[+] Acerca el tag origen al lector...{Colors.ENDC}")
                clf.connect(rdwr={'on-connect': lambda tag: clonar_tag(tag, None)})
                print(f"\n{Colors.OKGREEN}[+] Acerca el tag destino al lector...{Colors.ENDC}")
